...
int main(int argc, _TCHAR* argv[])
{
	...
	try
	{
		...
		serv.sin_port = htons(3000); // порт 2000
		serv.sin_addr.s_addr = inet_addr("127.0.0.1"); // адрес сервера 
		//serv.sin_addr.s_addr = inet_addr("172.20.10.4"); // адрес сервера 
		...
		if ((lbuf = sendto(cC, buf, strlen(buf) + 1, NULL, (sockaddr*)&serv, sizeof(serv))) == SOCKET_ERROR)
			throw SetErrorMsgText("send:", WSAGetLastError());
		if ((lbuf = recvfrom(cC, buf, sizeof(buf), NULL, (sockaddr*)&serv, &ls)) == SOCKET_ERROR)
